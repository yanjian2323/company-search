...
function get_company_id(html){
	...
	for(var i = 0, len = $trs.length; i < len; i++){
		...
		arr_company_id.push(company_id);
	}

	return arr_company_id;
}
...
function get_company_date(html){
	...
	var get_date = function(){
		for(var i = 0, len = $trs.length; i < len; i++){
			...
			for(var j = 0, len1 = $tds.length; j < len1; j++){
				if(flag){
					return ($tds.eq(j).text() || '').trim();
				}
				if($tds.eq(j).text().trim().indexOf('成立日期') > -1 || 
				 $tds.eq(j).text().trim().indexOf('注册日期') > -1){
					flag = true;
				}else{
					flag = false;
				}
...
function get_date(name, callback){
	new Seq()
		...
		.seq(function(arr_company_id){
			var arr_date = [];

			async.each(arr_company_id, function(company_id, next){
